...
@RestController
@RequestMapping("/api/v1/filesources")
public class FilesourceController {
	...
	public ResponseEntity<?> getAll() {
		List<Filesource> filesources = filesourceRepository.findAll();
		return ResponseEntity.status(HttpStatus.OK).body(filesources);
	}

	@GetMapping("/{id}")
	public ResponseEntity<?> getById(@PathVariable(value = "id", required = true) Long id) {
		Optional<Filesource> filesource = filesourceRepository.findById(id);
		if (filesource.isPresent()) {
			return ResponseEntity.status(HttpStatus.OK).body(filesource);
		}
		return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
	}

	@PostMapping
	public ResponseEntity<?> register(@RequestBody Filesource filesource) {
		try {
			filesourceService.register(filesource);
			return ResponseEntity.status(HttpStatus.CREATED).body(filesource);
